...
class Admin_Imagen_Controllers extends Controller
{
  ...
  public function borrar_esta_imagen($id)
  {
    ...
    HelpersGenerales::forgetThisCacheKey('ImagenPrincipal'. $Imagen->key_id . $Imagen->valor_id_del_campo_key);
      ...
      if(file_exists($Imagen->path_url_img_vertical))
      {
        unlink($Imagen->path_url_img_vertical);
      }

      if(file_exists($Imagen->path_url_img_vertical_chica))
      {
        unlink($Imagen->path_url_img_vertical_chica);
      }
